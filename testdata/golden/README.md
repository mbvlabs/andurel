...
- Under the tag, `layout.Scaffold` skips goose fix, templ generate, narsilc
  generate, `go mod tidy`, and `go fmt` **unless** `testseed.FullScaffold()` is
  true (`andurel_golden_full` tag or `ANDUREL_GOLDEN_FULL=1`). PR goldens stay
  offline; nightly full-tree `new` runs the generators with pinned tools on
  `ANDUREL_TOOL_BIN`.
...
`ANDUREL_GOLDEN_FULL=1` makes `layout.Scaffold` run goose fix, templ generate,
narsilc generate, `go fmt`, and `go mod tidy` with pinned tools from
`ANDUREL_TOOL_BIN`.
