...
func newLlmCommand() *cobra.Command {
	...
	cmd.AddCommand(newLlmDatastarCommand())
...
func newLlmDatastarCommand() *cobra.Command {
	return &cobra.Command{
		Use:   "datastar",
		Short: "Datastar usage in Andurel (client + server)",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			fmt.Println(llmDatastarDocumentation)
			return nil
		},
	}
}
...
- CRUD views generated by andurel generate view/resource include forms and actions wired for hypermedia.
  - Any data-* attributes used for Datastar-style interactivity are explained in: andurel llm datastar.
...
const llmDatastarDocumentation = `# Andurel Framework - Datastar (Client + Server)

Andurel uses Datastar-style attributes in views, but wraps the server-side protocol in the internal/hypermedia package. This doc separates client and server responsibilities.
