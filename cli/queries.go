...
const (
	...
	sqlcEffectiveRelativePath = "internal/storage/andurel_sqlc_config.yaml"
	sqlcGeneratedHeader       = "# generated by andurel; DO NOT EDIT\n"
	defaultSQLCOverlay        = `# User SQLC overlay.
#
# This file is merged on top of internal/storage/andurel_sqlc_config.yaml.
#
# Allowed scope:
# - You may only add fields under sql[].gen.
# - Do not set/modify sql[].schema, sql[].queries, sql[].engine, sql[].version,
#   or framework-managed gen.go.* defaults (package/out/output file names/sql_package).
#
# Validate and apply overlay with: andurel queries validate
`
)
...
func sqlcEffectivePath(rootDir string) string {
	return filepath.Join(rootDir, sqlcEffectiveRelativePath)
}

func initQueriesSQLCFiles(rootDir string) ([]string, error) {
	basePath := sqlcEffectivePath(rootDir)
	...
	if _, err := ensureEffectiveSQLCConfig(rootDir); err != nil {
		return nil, err
	}
...
func validateQueriesSQLCFiles(rootDir string) error {
	_, err := ensureEffectiveSQLCConfig(rootDir)
	return err
}

func ensureEffectiveSQLCConfig(rootDir string) (string, error) {
	effectivePath := sqlcEffectivePath(rootDir)
	if _, err := os.Stat(effectivePath); err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return "", fmt.Errorf("missing %s", effectivePath)
		}
		return "", fmt.Errorf("failed to read effective sqlc config: %w", err)
	}

	baseMap, err := readYAMLAsMap(effectivePath)
	if err != nil {
	...
	if err := validateSQLCOverlay(userMap); err != nil {
		return "", err
	}

	merged := mergeYAMLMaps(baseMap, userMap)
	if err := validateMergedSQLCConfig(merged); err != nil {
		return "", err
	}

	encoded, err := marshalCanonicalSQLC(merged)
	if err != nil {
		return "", fmt.Errorf("failed to encode effective sqlc config: %w", err)
	}
	if err := os.WriteFile(effectivePath, []byte(sqlcGeneratedHeader+string(encoded)), 0o644); err != nil {
...
func runSqlcValidate() error {
	...
	effectivePath, err := ensureEffectiveSQLCConfig(rootDir)
	if err != nil {
		return err
	}
