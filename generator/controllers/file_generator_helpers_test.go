...
`
	...
	if strings.Contains(merged, `fmt.Println("generated")`) ||
		strings.Count(merged, "type Products struct") != 1 {
		t.Fatalf("existing declarations were replaced or duplicated:\n%s", merged)
	}

	withoutImports, err := mergeControllerSources(
		"package controllers\ntype Products struct{}\n",
	...
	if _, err := mergeControllerSources(
		"package controllers\nfunc (",
		generated,
	); err == nil ||
		!strings.Contains(err.Error(), "existing controller") {
	...
	if _, err := mergeControllerSources(
		...
		!strings.Contains(err.Error(), "generated controller") {
		t.Fatalf("expected generated parse error, got %v", err)
	}
}

func TestControllerFrontendActionsAndRegistrationHelpers(t *testing.T) {
	if got := detectControllerFrontend(
		`import "example.com/internal/inertia"`,
	); got != controllerFrontendInertia {
		t.Fatalf("inertia frontend = %q", got)
	}
	if got := detectControllerFrontend(
		`import "example.com/pkg/hypermedia"`,
	); got != controllerFrontendTempl {
...
`
	...
	if _, err := filterControllerActions(
		"package controllers\nfunc (",
		[]string{"index"},
	); err == nil {
