...
func TestSQLiteGenerateConversionToDB(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			"string",
			"entity.Name",
			"sql.NullString{String: entity.Name, Valid: true}",
		},
		{
			"NullInt64 to DB",
			"sql.NullInt64",
			"int64",
			"entity.Age",
			"sql.NullInt64{Int64: entity.Age, Valid: true}",
		},
		{
			"NullFloat64 to DB",
			"sql.NullFloat64",
			"float64",
			"entity.Price",
			"sql.NullFloat64{Float64: entity.Price, Valid: true}",
		},
		{
			"NullBool to DB",
			"sql.NullBool",
			"bool",
			"entity.Active",
			"sql.NullBool{Bool: entity.Active, Valid: true}",
		},
		{
			"NullTime to DB",
			"sql.NullTime",
			"time.Time",
			"entity.CreatedAt",
			"sql.NullTime{Time: entity.CreatedAt, Valid: true}",
		},
...
func TestSQLiteConversionsIntegration(t *testing.T) {
	...
	if goType != "string" || sqlcType != "sql.NullString" || pkg != "database/sql" {
		t.Errorf(
			"MapSQLTypeToGo(text, true) = (%s, %s, %s), want (string, sql.NullString, database/sql)",
			goType,
			sqlcType,
			pkg,
		)
