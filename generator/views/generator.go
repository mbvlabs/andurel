...
func (g *Generator) Build(cat *catalog.Catalog, config Config) (*GeneratedView, error) {
	...
	for _, col := range table.Columns {
		...
		if col.IsPrimaryKey {
			...
			view.IDJSONName = types.FormatCamelCase(col.Name)
			continue
		}
		if col.Name == "id" {
			continue
...
func inertiaReactEditValue(field ViewField) string {
	return inertiaEditValue(field, "item")
}

func inertiaEditValue(field ViewField, objectRef string) string {
	fieldName := inertiaJSONFieldName(field)
	switch inertiaReactFieldType(field) {
	case "boolean":
		return "Boolean(" + objectRef + "." + fieldName + ")"
	case "number":
		return "Number(" + objectRef + "." + fieldName + " ?? 0)"
	default:
		value := "String(" + objectRef + "." + fieldName + " ?? '')"
...
func inertiaReactDisplay(field ViewField, objRef string) string {
	ref := objRef + "." + inertiaJSONFieldName(field)
...
func inertiaJSONFieldName(field ViewField) string {
	if field.CamelCase != "" {
...
func inertiaTypeScriptType(field ViewField) string {
	switch resolveViewBaseType(field.GoType) {
	case "bool":
		return "boolean"
...
func inertiaPayloadTypeName(view *GeneratedView) string {
	return view.NamespacePascal + view.ResourceName + "Data"
}

func inertiaIDJSONName(view *GeneratedView) string {
	if view.IDJSONName != "" {
		return view.IDJSONName
	}
	return naming.ToLowerCamelCase(view.IDFieldName)
}

func inertiaPayloadImportPath(view *GeneratedView) string {
	return "@/types/" + namespacePrefix(view.Namespace) + naming.ToSnakeCase(view.ResourceName)
}

func inertiaTypeScriptDeclarations(view *GeneratedView) string {
	var b strings.Builder
	b.WriteString("// Code generated by Andurel; DO NOT EDIT.\n\n")
	fmt.Fprintf(&b, "export type %s = {\n", inertiaPayloadTypeName(view))
	idType := "string"
	if view.IDType == "int32" || view.IDType == "int64" {
		idType = "number"
	}
	fmt.Fprintf(&b, "  %s: %s\n", inertiaIDJSONName(view), idType)
	for _, field := range view.Fields {
...
func (g *Generator) GenerateInertiaViewFiles(
	...
	customFuncs := template.FuncMap{
		...
		"InertiaFieldType":   inertiaReactFieldType,
		"InertiaCreateValue": inertiaReactCreateValue,
		"InertiaEditValue":   inertiaEditValue,
		...
		"InertiaPayloadType": func() string {
			return inertiaPayloadTypeName(view)
		},
		"InertiaPayloadImportPath": func() string {
			return inertiaPayloadImportPath(view)
		},
		"InertiaIDJSONName": func() string {
			return inertiaIDJSONName(view)
		},
...
func (g *Generator) generateInertiaViews(
	...
	typesDir := filepath.Join("resources", "js", "types")
