...
func CreateUser(
	...
	params := db.NewInsertUserParams(
		...
		"", // phone field added by schema migration
	)
...
func UpdateUser(
	...
	params := db.NewUpdateUserParams(
		...
		func() string {
			return "" // phone field added by schema migration
		}(),
	)
...
func CustomUserHelper(id uuid.UUID) string {
	return "custom-" + id.String()
}

func GetActiveUsersCount(ctx context.Context, dbtx db.DBTX) (int64, error) {
	// Custom business logic function that should be preserved
	users, err := AllUsers(ctx, dbtx)
	if err != nil {
		return 0, err
	}
