...
func (u *Upgrader) buildPlan(dirty bool) (*migrationPlan, error) {
	...
	for _, path := range paths {
		if recognized, recognitionErr := recognizeWholeFileReplacement(
			u.projectRoot, path, rendered[path], u.lock.Version, u.opts.TargetVersion,
			modulePath, u.lock.ScaffoldConfig.ProjectName,
		); recognitionErr != nil {
	...
	if len(plan.conflicts) == 0 {
		if err := validatePlannedFiles(u.projectRoot, plan); err != nil {
			return nil, fmt.Errorf("validate planned upgrade: %w", err)
		}
	}
	return plan, nil
}

func recognizeWholeFileReplacement(
	root, path string,
	target []byte,
	...
	if bytes.Equal(
		normalizeGeneratedBanner(current, sourceVersion),
		normalizeGeneratedBanner(target, targetVersion),
	) {
		return true, nil
	}
...
func normalizeGeneratedBanner(content []byte, version string) []byte {
	if version == "" {
		return slices.Clone(content)
	}
	exact := "// Code generated by andurel " + version + "; DO NOT EDIT."
	...
	for index, line := range lines {
		...
		if bytes.Equal(body, []byte(exact)) {
			lines[index] = append([]byte("// Code generated by andurel <FRAMEWORK_VERSION>; DO NOT EDIT."), ending...)
		}
	}
