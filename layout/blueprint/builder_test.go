...
func TestBuilder_AddControllerDependency(t *testing.T) {
	...
	builder.
		...
		AddCtrlDependency("cache", "cache.Cache").
...
func TestBuilder_AddRoute(t *testing.T) {
	...
	route1 := Route{Name: "home", Path: "/", Handler: "Pages.Home"}
	route2 := Route{Name: "about", Path: "/about", Handler: "Pages.About"}
...
func TestBuilder_AddMigration(t *testing.T) {
	...
	mig1 := Migration{Name: "001_init", Timestamp: "20250101000000"}
	mig2 := Migration{Name: "002_users", Timestamp: "20250102000000"}
...
func TestBuilder_Chaining(t *testing.T) {
	...
	result := builder.
		...
		AddControllerField("Pages", "Pages").
		AddConstructor("pages", "newPages(db)").
		AddRoute(Route{Name: "home", Path: "/"}).
		AddRouteImport("middleware").
		AddModel(Model{Name: "User"}).
		AddModelImport("time").
		AddConfigField("Port", "int").
		AddEnvVar("PORT", "Port", "8080").
		AddMigration(Migration{Name: "001_init"})
...
func TestBuilder_EmptyValues(t *testing.T) {
	...
	builder.
		...
		AddConstructor("", "expr").
		AddConstructor("var", "").
		AddRoute(Route{Name: "", Path: "/"}).    // missing name
		AddRoute(Route{Name: "name", Path: ""}). // missing path
		AddRouteImport("").
		AddModel(Model{Name: ""}). // missing name
		AddModelImport("").
		AddConfigField("", "Type").
		AddConfigField("name", "").
		AddEnvVar("", "field", "default").
		AddEnvVar("key", "", "default").
		AddMigration(Migration{Name: ""}) // missing name
