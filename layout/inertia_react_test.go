...
func TestScaffoldReactInertiaAssets(t *testing.T) {
	...
	assertFileContains(t, projectDir, "cmd/app/main.go", `func newInertia(lifecycle fx.Lifecycle, cfg config.InertiaCfg, metadata application.Metadata) (*inertia.Renderer, error)`)
	assertFileContains(t, projectDir, "cmd/app/main.go", `inertia.WithRoot(views.Root)`)
	assertFileContains(t, projectDir, "cmd/app/main.go", `inertia.WithEntryPoint("resources/js/app.tsx")`)
